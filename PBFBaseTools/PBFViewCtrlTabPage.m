...
#import "PBFViewCtrlTabPage.h"
#import "PBFColorTools.h"
#import "PBFConvert.h"

#define kBOYViewCtrlSimulateTabHeight       40

@interface PBFViewCtrlTabPage ()
...
- (void)viewDidLoad {
    ...
    //设置按钮背景
    rect.size.height = kBOYViewCtrlSimulateTabHeight;
    ...
    //加上按钮
    for( int i = 0 ; i < [_ARRCtrl count] ; ++i ){
        rectButton.size.height = kBOYViewCtrlSimulateTabHeight;
        if (isPassWidth) {
...
- (UIView*)ViewSelTab{
    if (!_ViewSelTab) {
        _ViewSelTab = [[UIView alloc] initWithFrame:CGRectMake(10, kBOYViewCtrlSimulateTabHeight-2, _INWidth - 20, 2)];
...
- (UIScrollView*)SCRContentView{
    if (!_SCRContentView) {
        ...
        rect.size.height -= kBOYViewCtrlSimulateTabHeight;
        rect.origin.y = kBOYViewCtrlSimulateTabHeight;
